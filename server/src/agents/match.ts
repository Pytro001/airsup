...
export async function processMatch(matchId: string): Promise<void> {
  ...
  const response = await anthropic.messages.create({
    ...
    max_tokens: 512,
    system: "Generate a concise context summary and next steps for a buyer-factory introduction. Respond with JSON: { \"summary\": \"...\", \"next_steps\": \"...\", \"key_context_for_buyer\": \"...\", \"key_context_for_factory\": \"...\" }",
    messages: [
      {
        role: "user",
...
Generate the introduction context.`,
  ...
  try {
    ...
    intro = { summary: text, next_steps: "Discuss details on WhatsApp.", key_context_for_buyer: "", key_context_for_factory: "" };
  }
  ...
  await supabaseAdmin.from("matches").update({
    ...
    context_summary: {
      ...
      factory_context: intro.key_context_for_factory,
